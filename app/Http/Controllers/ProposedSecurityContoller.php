...
class ProposedSecurityContoller extends Controller {
	...
	public function store(ProposedSecurityRequest $request)
	{
        if( $request->apprequest ==0) //create new profile
        {
            ...
            $ps->rate_applied=$request->rate_applied;
            $ps->save();
            ...
            $open_marketvalue = $request->open_value;
            $forced_salevalue = $request->forced_value;
            $tobe_charges = $request->tobe_charged;
            ...
            for ($i = 0; $i < sizeof($security_type); $i++) {
