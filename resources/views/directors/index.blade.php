            ...
            <div class="col-md-12">
                <div class="widget box">
                    <div class="widget-header">
                        <h4><i class="icon-reorder"></i>  DirectorS Credit Committee</h4>
                            ...
                            <tbody>
                               ...
                               @if(count($cm) > 0)
                                   @foreach($cm as $ct)
                                       <?php $c++;
                                       $status="";
                                               if($ct->status ==0){$status="Active";}else{$status="Inactive";}
                                       ?>
                               <tr>
                                   <td>{{$c }}</td>
                                   <td>{{$ct->firstname }}</td>
                                   <td><span class="btn btn-success btn-block">{{ucwords(strtolower($status)) }}</span></td>
                                   <td id="{{ $ct->id }}">
                                       <a href="{{url('directors/edit')}}/{{ $ct->id }}" title="edit Application" class="editapp "><i class="icon-pencil text-info"></i> edit</a>&nbsp;&nbsp;&nbsp;
                                       <a href="{{url('directors/delete')}}/{{ $ct->id }}" title="delete Application" class="deleteapp "><i class="icon-trash text-danger"></i> delete </a>
                                   </td>
                               </tr>
                               @endforeach
                                   @endif
