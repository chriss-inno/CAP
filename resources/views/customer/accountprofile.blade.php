...
$crid=$customer->id;
        ...
        if( count($customer->accountprofile) !=0)
        {
            $apprequest =1;

            $borrowerid=$customer->accountprofile->borrowerid;
            $credit_rating=$customer->accountprofile->credit_rating;
            $legal_entity=$customer->accountprofile->legal_entity;
            $business_activity =$customer->accountprofile->business_activity;
            $g_indicator =$customer->accountprofile->g_indicator;
            $group =$customer->accountprofile->app_group;
            $emanagement =$customer->accountprofile->emanagement;

            $shareholders =$customer->shareholders;
            $directors =$customer->directors;
            $cr_bankers =$customer->currentbankers;
            $id= $customer->accountprofile->id;
        }
